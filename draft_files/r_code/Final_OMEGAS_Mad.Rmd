---
title: "Apply GREEN and RED calibration data to Madgetech readings"
...
filename_mad_green <- "green_mad_25.2"
filename_mad_red <- "red_mad_25.2" #5.1, 5.2, 10, 20, 25, 25.2
temperature <- "25.2"

#apply temperature correction equations
...
## Run "Ideal" calibration for Madgetech GREEN deployment data
```{r}
#load calibration data
calibration_values <- read_csv(here("data", "calibrated", paste(filename_green_tris,".csv",sep=""))) # read in tris data excel file

rawdata_all <- read_csv(here("data", "raw", "april-green", paste(filename_mad_green,".csv",sep=""))) %>% # read in tris data excel file
...
calibration_values <- raw_calibrated %>%
...
#Write .csv
write.csv(calibration_values, here("data", "calibrated", paste(filename_mad_green,"_omegas.csv", sep="")),row.names = FALSE) # create csv with calibrated values
```

## Run "Ideal" calibration for Madgetech RED deployment data
```{r}
#load calibration data
calibration_values <- read_csv(here("data", "calibrated", paste(filename_red_tris,".csv",sep=""))) # read in tris data excel file

rawdata_all <- read_csv(here("data", "raw", "april-red", paste(filename_mad_red,".csv",sep=""))) %>% # read in tris data excel file
  filter(row_number()!=c(1:5)) %>% # remove the first 5 rows
  row_to_names(row_number=1) %>% # make the top row the column headers
  rename("Date"=1,
         "Time"=2) %>% # rename date and time columns
...
#Write .csv
write.csv(calibration_values, here("data", "calibrated", paste(filename_mad_red,"_omegas.csv", sep="")),row.names = FALSE) # create csv with calibrated values
```
