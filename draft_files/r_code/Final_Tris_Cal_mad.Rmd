---
title: "Calculate calibration constants for RED and GREEN DuraFETs on MADGETECH"
...
filename_green <- "green_mad_25" #filename for madgetech green durafet tris data
filename_red <- "red_mad_25" #filename for madgetech red durafet tris data
...
```{r}
#Run the code
trisdata <- read_csv(here("data", "raw", "april-green", paste(filename_green,".csv",sep=""))) %>%
...
```{r}
#Run the code
trisdata <- read_csv(here("data", "raw", "april-red", paste(filename_red,".csv",sep=""))) %>%
    filter(row_number()!=c(1:5)) %>% # remove the first 5 rows
  row_to_names(row_number=1) %>% # make the top row the column headers
  rename("Date"=1,
         "Time"=2) %>% # rename date and time columns
