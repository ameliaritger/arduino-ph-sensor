...
void loop()
{
  ...
  if (alarmTrigger && rtc.alarm(DS3232RTC::ALARM_1)) { // check alarm flag (and clear the flag if set)
    ...
    if (datafile) { // if the file opened okay, write to it:
      ...
      // Oversample
      Serial << "J4+ oversampled: " << oversampleAverage(adc2_1) << "(" << oversampleAverage(adc2_1) * ADS1115_GAIN_MULT << "mV)" << endl;
      Serial << "J4+ oversampled (min): " << oversampleMinimum(adc2_1) << "(" << oversampleMinimum(adc2_1) * ADS1115_GAIN_MULT << "mV)" << endl;
      Serial << "J4+ oversampled (max): " << oversampleMaximum(adc2_1) << "(" << oversampleMaximum(adc2_1) * ADS1115_GAIN_MULT << "mV)" << endl;

      Serial << "J4- oversampled: " << oversampleAverage(adc2_2) << "(" << oversampleAverage(adc2_2) * ADS1115_GAIN_MULT << "mV)" << endl;
      Serial << "J4- oversampled (min): " << oversampleMinimum(adc2_2) << "(" << oversampleMinimum(adc2_2) * ADS1115_GAIN_MULT << "mV)" << endl;
      Serial << "J4- oversampled (max): " << oversampleMaximum(adc2_2) << "(" << oversampleMaximum(adc2_2) * ADS1115_GAIN_MULT << "mV)" << endl;

      Serial << "J1 oversampled: " << oversampleAverage(adc2_diff) << "(" << oversampleAverage(adc2_diff) * ADS1115_GAIN_MULT << "mV)" << endl;
      Serial << "J1 oversampled (min): " << oversampleMinimum(adc2_diff) << "(" << oversampleMinimum(adc2_diff) * ADS1115_GAIN_MULT << "mV)" << endl;
      Serial << "J1 oversampled (max): " << oversampleMaximum(adc2_diff) << "(" << oversampleMaximum(adc2_diff) * ADS1115_GAIN_MULT << "mV)" << endl;

      // Write ADC inputs
      datafile << adc2_diff * ADS1115_GAIN_MULT << "," << oversampleAverage(adc2_diff) * ADS1115_GAIN_MULT << ","; //J1
      datafile << adc1_1 * ADS1015_GAIN_MULT << ","; //J2
      datafile << adc1_diff * ADS1015_GAIN_MULT << ","; //J3
      datafile << (oversampleAverage(adc2_1) * ADS1115_GAIN_MULT) - (oversampleAverage(adc2_2) * ADS1115_GAIN_MULT) << "," << (adc2_1 * ADS1115_GAIN_MULT) - (adc2_2 * ADS1115_GAIN_MULT) << ","; //J4
      datafile << adc2_1 * ADS1115_GAIN_MULT << "," << adc2_2 * ADS1115_GAIN_MULT << ","; //J4+ and J4-
      datafile << oversampleAverage(adc2_1) * ADS1115_GAIN_MULT << "," << oversampleAverage(adc2_2) * ADS1115_GAIN_MULT << endl; //J4+ and J4- oversampled
