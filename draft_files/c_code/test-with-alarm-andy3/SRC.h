...
// Function to oversample ADCs and generate array of oversampled values
int oversampleADC(int16_t adcInput) {
  double oversampleArray[OVERSAMPLE_RATE]; // create array to fill oversample data, size of the array is the oversampling rate value 
  for (int i = 0; i < OVERSAMPLE_RATE; i++) {
    int adcValue = adcInput;
    oversampleArray[i] = adcValue; // add the adcValue to oversampleArray
  }
  return 0; //how do I return the array?
}
