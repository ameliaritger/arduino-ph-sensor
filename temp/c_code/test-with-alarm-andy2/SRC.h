...
// Function to oversample ADC inputs and return average value
float oversampleAverage(int16_t adcInput) {
  //int oversampleRate = OVERSAMPLE_RATE;
  float sumSample = 0;
  for (int i = 0; i < OVERSAMPLE_RATE; i++) {
    int adcValue = adcInput;
    sumSample += adcValue;
  }
  int averageSample = sumSample / OVERSAMPLE_RATE;
  return averageSample;
}

// Function to oversample ADC inputs and return standard deviation
float oversampleVariance(int16_t adcInput) {
  int oversampleArray[OVERSAMPLE_RATE]; // create array to fill oversample data
  int sdArray = oversampleArray[0];
  for (int i = 0; i < OVERSAMPLE_RATE; i++) {
    int adcValue = adcInput;
    oversampleArray[i] = adcValue; // add the adcValue to oversampleArray
  }
  float sdVal = standardDeviation(oversampleArray, OVERSAMPLE_RATE);
  return sdVal;
}

// Function to oversample ADC inputs and return minimum value
float oversampleMinimum(int16_t adcInput) {
  int oversampleArray[OVERSAMPLE_RATE]; // create array to fill oversample data
  int minVal = oversampleArray[0];
  for (int i = 0; i < OVERSAMPLE_RATE; i++) {
    int adcValue = adcInput;
    oversampleArray[i] = adcValue; // add the adcValue to oversampleArray
  }
  for (int j = 0; j < (sizeof(oversampleArray) / sizeof(oversampleArray[0])); j++) {
    minVal = min(oversampleArray[j], minVal);
  }
  return minVal;
}

// Function to oversample ADC inputs and return maximum value
float oversampleMaximum(int16_t adcInput) {
  int oversampleArray[OVERSAMPLE_RATE]; // create array to fill oversample data
  int maxVal = oversampleArray[0];
  for (int i = 0; i < OVERSAMPLE_RATE; i++) {
    int adcValue = adcInput;
    oversampleArray[i] = adcValue; // add the adcValue to oversampleArray
  }
  for (int j = 0; j < (sizeof(oversampleArray) / sizeof(oversampleArray[0])); j++) {
    maxVal = max(oversampleArray[j], maxVal);
  }
  return maxVal;
}
