...
void setup() {
  ...
  rtc.begin();
  //setSyncProvider(rtc.get); // Time and date are set to date and time on your computer at compiletime
  clearClockTrigger();
...
void clearClockTrigger() {  // from http://forum.arduino.cc/index.php?topic=109062.0
  ...
  Wire.read();      //In this example we are not interest in actually using the byte
  Wire.beginTransmission(DS3231_I2C_ADDRESS);   //Tell devices on the bus we are talking to the DS3231
...
void enableRTCAlarmsonBackupBattery() {
  ...
  bitSet(resisterData, 1);                   // Change bit 6 to a 1 to enable
  Wire.beginTransmission(DS3231_I2C_ADDRESS);// Attention RTC
