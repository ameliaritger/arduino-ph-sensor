...
//edits: 12/14/2022 made it sample every 5 minutes and set time every new upload; changed delay() to millis() 
...
#define FILE_NAME "1215depl.csv" //KEEP THIS AT OR UNDER 8 CHARACTERS DUMMY
...
//Set variables for sampling rate
const int ledPin = LED_BUILTIN;
int ledState = LOW;             // ledState used to set the LED
unsigned long previousMillis = 0;     // will store last time time was updated
const long interval = 300000;           // interval at which to perform function (milliseconds)

//int delayTime = 497500; //take a reading every X/1000 seconds (plus delayBlink time) take reading once every 5 minutes
//int delayBlink = 250; //blink on/off every X/1000 seconds
int sampleRate = 64; //16, (32) 64, (128) 256 //add 1000,2000,7000 (seconds) to delayTime for total sampling rate
...
void loop()
{
  ...
  if (currentMillis - previousMillis >= interval) {
    previousMillis = currentMillis; // save the last time you took reading
