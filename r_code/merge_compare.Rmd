...
merged_diff <- merged %>%
  ...
  mutate(diff_time = date_time-lag(date_time, default = first(date_time)),
         ...
         diff_ph = omegasph-lag(omegasph, default=first(omegasph))) %>%
  ungroup() %>%
  filter(!diff_temp == 0) %>%
  group_by(period, logger) %>%
  summarize(mean_diff_t = mean(diff_temp),
            mean_temp = mean(durafet_temp),
            mean_diff_p = mean(diff_ph))

#avg1 <- mean(0.1450744,0.1525867) get these numbers from above
#avg2 <- mean(0.1721097,0.1695504)
#avg_temp <- avg2-avg1

#avg3 <- mean(0.2472605,0.2480029)
#avg4 <- mean(0.2536188,0.2536685) get these numbers from above
#avg_ph <- avg4-avg3
```
