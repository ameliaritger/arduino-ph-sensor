...
//edits: 12/13/2022 fixed subtraction error with J4 oversampling calculation and create .csv not .txt
...
#define FILE_NAME "over_raw.csv" //KEEP THIS AT OR UNDER 8 CHARACTERS DUMMY
...
//int delayTime = 9500; //take a reading every X/1000 seconds (plus delayBlink time)
...
void setup()
{
  // initialize digital pin 13 AKA LED_BUILTIN as an output (blink)
  ...
  if (SD.exists(FILE_NAME)) {
    ...
    datafile.println("Date,Time,J1 (mV)");
    datafile.close();
    Serial.println("done.");
  }
...
void loop()
{
  ...
  if (datafile){
    ...
    for(int i = 0; i < over_samples_one; i++){
      ...
      //Serial.print(a);
      datafile.print(','); datafile.print(','); datafile.print(a * ADS1115_GAIN); datafile.print(','); datafile.println(sampleRate);
      sum_samples_one += a;
    }
