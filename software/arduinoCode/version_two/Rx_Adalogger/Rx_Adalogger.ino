...
void setup() {
  ...
  LowPower.deepSleep(); // go to sleep until Alarm is triggered. This will break Serial communications, but the loop will still be running.
}
...
void loop() {
  ...
  if (goToSleep) {
    LowPower.deepSleep(); // go to sleep. This will break Serial communications, but the loop will still be running.
  }
}
